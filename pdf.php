...
$query = "SELECT DISTINCT s.usuario  from cat_usuario s join detalle_temporal_pres d ON (s.`id_usuario` = d.`id_usuario`) where d.`id_usuario`=s.`id_usuario`"; 
if($resul = $conexion->query($query))
while ($rew = $resul->fetch_assoc()) {
$pdf->SetFont('Arial','',8);
$pdf->Cell(55,10,utf8_decode($rew['usuario']),0,1,'L');
	}
$pdf->SetFont('Arial','B',10);
